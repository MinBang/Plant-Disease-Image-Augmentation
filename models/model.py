...
class CycleGAN:
    ...
    def build_model(self, args):
        ...
        nets.G_A2B = self.define_network(ResnetGenerator(args.input_nc, args.output_nc), init_func)
        nets.G_B2A = self.define_network(ResnetGenerator(args.input_nc, args.output_nc), init_func)        
        
        nets.D_A = self.define_network(NLayerDiscriminator(args.input_nc), init_func)
        nets.D_B = self.define_network(NLayerDiscriminator(args.input_nc), init_func)       
    ...
    def backward_D(self):
        fake_A = self.buffer.A_replay.push_and_pop(self.fake_A)
        self.loss_D_A = self.backward_D_basic(self.nets.D_A, self.real_A, fake_A)

        fake_B = self.buffer.B_replay.push_and_pop(self.fake_B)
        self.loss_D_B = self.backward_D_basic(self.nets.D_B, self.real_B, fake_B)
        self.loss_D = self.loss_D_A + self.loss_D_B / 2

    def backward_G(self):
        self.loss_G_A = self.criterion_GAN(self.nets.D_A(self.fake_A), True) ## Pair netG_B2A
        self.loss_G_B = self.criterion_GAN(self.nets.D_B(self.fake_B), True) ## Pair netG_B2A
        self.loss_gan = self.loss_G_A + self.loss_G_B
